...
int Init()
{
	...
	WindowHandle hPrimaryWindow = CreateWindow(c_iDefaultScreenWidth, 
            c_iDefaultScreenHeight, "First", nullptr, nullptr);
	...
	int iOpenGLMajor = glfwGetWindowAttrib(hPrimaryWindow->m_pWindow, 
                                            GLFW_CONTEXT_VERSION_MAJOR);
	int iOpenGLMinor = glfwGetWindowAttrib(hPrimaryWindow->m_pWindow, 
                                            GLFW_CONTEXT_VERSION_MINOR);
	int iOpenGLRevision = glfwGetWindowAttrib(hPrimaryWindow->m_pWindow,
        ...
        */
		...
		for (const auto& window : g_lWindows)
		{
			...
			glUniformMatrix4fv(ProjectionID, 1, false,
                    glm::value_ptr(window->m_m4Projection) );
	...
	for (auto& itr : g_lWindows)
	{
		if (itr->m_pWindow == a_pWindow)
		{
			...
			window->m_m4Projection = glm::perspective(45.0f, 
                    float(a_iWidth)/float(a_iHeight), 0.1f, 1000.0f);
		}
	}
